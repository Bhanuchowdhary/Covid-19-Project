...
select location
, max(total_cases) as no_of_cases
from covid.deaths
...
select continent
, sum( total_cases_by_loc) as total_cases_by_continent
from 
( 
select continent
, location
,max(total_cases) as total_cases_by_loc 
from covid.deaths 
...
select location
, max(cast(total_deaths as float)) as no_of_deaths 
from covid.deaths
...
select loc.continent as continent
, sum(deaths_by_loc) as no_of_deaths 
from (
select continent
,location
,max(cast(total_deaths as float)) as deaths_by_loc 
from covid.deaths
group by location
) as loc
...
,sum(cast(new_deaths as float)) as deaths
...
select *
, total_people_vaccinated/population*100 as 'percent of people vaccinated' 
from dcte;
...
select *
, total_people_vaccinated/population*100 as 'percentage of people vaccinated'
from temp_table;
